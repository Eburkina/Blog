        ...
        <div class="card">
          ...
          <form action="{{route('categorie-store')}}" method="POST">
            ...
            <div class="card-body">
                ...
                <button type="button" class="btn btn-danger btn-sm mr-2 text-light  mod" data-toggle="modal" data-target="#supp{{$categorie->uuid}}"><i class="fas fa-trash-alt text-light"></i>Supprimer</button>
                </div>
                @endif
            </div>
        </form>
    @if ($categorie->uuid != null)
     @include('vendor.partials._confirmation_modal2', ['modalUrl'=>'categorie-delete','uuid'=>$categorie->uuid,'idModal' => "supp".$categorie->uuid ,'message'=>'Vous êtes sur le point de supprimer la catégorie '.$categorie->titre])
    @endif
    </div>
    </div>
</section>
